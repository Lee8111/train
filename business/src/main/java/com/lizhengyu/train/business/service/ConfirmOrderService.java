...
public class ConfirmOrderService {
     ...
     * 例子:sell=10001,本次购买区间站：1~4,则区间已售000
     * 全部是0，表示这个区间可买，只要有1，就表示区间内已售过票
     *
     * 选中后，要计算购票后的sell，比如原来是10001，本次购买区间站：1~4
    ...
    public boolean calSell(DailyTrainSeat dailyTrainSeat,Integer startIndex,Integer endIndex){
        ...
        if(Integer.parseInt(sellPart)>0){
            LOG.info("座位{}在本次车站区间{}~{}已售过票，不可选中这座位",dailyTrainSeat.getCarriageIndex(),startIndex,endIndex);
            return false;

        }else{
            LOG.info("座位{}在本次车站区间{}~{}未售过票，可选中这座位",dailyTrainSeat.getCarriageIndex(),startIndex,endIndex);
            //111 ,111
